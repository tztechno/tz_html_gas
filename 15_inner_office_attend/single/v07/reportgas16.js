...
<body>
    <div class="container">
        ...
        <div class="button-group">
            <button onclick="resetData()" class="button-danger">出勤データリセット</button>
        </div>
...
// generateReportPreview関数の修正
function generateReportPreview(dateStr, company) {
    const data = getAttendanceData();
    const { presentEmployees, unreportedEmployees } = categorizeEmployees(data, dateStr, company);
...
// sendManualAttendanceReport関数の修正
function sendManualAttendanceReport(dateStr, company) {
    try {
        ...
        MailApp.sendEmail({
            ...
            subject: `出勤状況レポート - ${companyName} (${dateStr})`,
            body: emailBody
        });
...
function categorizeEmployees(data, dateStr, company) {
    ...
    rows.forEach(row => {
        const employeeCompany = row[0];  // 会社
        const employeeName = row[1];     // 名前
        const arrivalTime = row[2];      // 出勤時刻
        const comment = row[3] || '';    // コメント列
        ...
        const employeeInfo = {
            name: `${employeeCompany} ${employeeName}`,
            comment: comment
        };

        if (arrivalTime) {
            presentEmployees.push(employeeInfo);
        } else {
            unreportedEmployees.push(employeeInfo);
        }
    });

    return {
        presentEmployees: [...new Set(presentEmployees)].sort((a, b) => a.name.localeCompare(b.name, 'ja')),
        unreportedEmployees: [...new Set(unreportedEmployees)].sort((a, b) => a.name.localeCompare(b.name, 'ja'))
    };
}

// createEmailBody関数の修正
function createEmailBody(presentEmployees, unreportedEmployees, dateStr, currentTime, company) {
...
${presentEmployees.map(emp => `${emp.name}${emp.comment ? ` (${emp.comment})` : ''}`).join('\n')}

【未出勤】 ${unreportedEmployees.length}名
${unreportedEmployees.map(emp => `${emp.name}${emp.comment ? ` (${emp.comment})` : ''}`).join('\n')}
`;
}

// Add new function to reset attendance data
function resetAttendanceData() {
    try {
        const dataSpreadsheet = SpreadsheetApp.openById(DATA_SPREADSHEET_ID);
        const dataSheet = dataSpreadsheet.getSheetByName('today');

        if (!dataSheet) {
            throw new Error('todayシートが見つかりません。');
        }

        // Get the last row with data
        const lastRow = dataSheet.getLastRow();
        if (lastRow > 1) {  // Only proceed if there's data beyond the header row
            // Clear columns C and D (starting from row 2 to preserve headers)
            dataSheet.getRange(2, 3, lastRow - 1, 2).clearContent();
        }

        return '出勤データのリセットが完了しました。';
    } catch (error) {
        console.error('Error in resetAttendanceData:', error);
        throw new Error(`リセット失敗: ${error.message}`);
    }
}
